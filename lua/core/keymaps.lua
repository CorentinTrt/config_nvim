...
vim.keymap.set("n", "<leader>ft", ":Neotree reveal left<CR>", { desc = "Toggle neo-tree" })
vim.keymap.set("n", "<leader>ff", ":Telescope find_files<CR>", { desc = "Find (files)" })
vim.keymap.set("n", "<leader>fg", ":Telescope live_grep path=%:p:h select_buffer=true<CR>", { desc = "Find (grep)" })

-- lsps (<leader>l)
vim.keymap.set("n", "<leader>lh", vim.lsp.buf.hover, { desc = "Show the documentation of element under cursor" })
vim.keymap.set("n", "<leader>la", vim.lsp.buf.code_action, { desc = "Show the available code actions" })
